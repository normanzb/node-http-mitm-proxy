...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  socket.on('error', function(err){
    if (err.code === 'ECONNRESET') {
      // usually the browser sends this to indicates close of socket
      // connection
      // TODO: avoid further operation with socket?
      return;
    } else {
      self._onError.call(self, 'CLIENT_TO_PROXY_SOCKET_ERROR', null, err);
    }
  });
